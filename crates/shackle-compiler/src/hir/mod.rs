...
pub struct Model {
	...
	/// Dimension items
	pub dimensions: Arena<Item<Dimension>>,
	/// Unit items
	pub units: Arena<Item<Unit>>,
}
...
impl_index!(Model[self, index: ArenaIndex<Item<Dimension>>] -> Item<Dimension> { self.dimensions[index] });
impl_index!(Model[self, index: ArenaIndex<Item<Unit>>] -> Item<Unit> { self.units[index] });
