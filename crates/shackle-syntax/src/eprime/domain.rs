//! Eprime Domain Expressions

use super::{Children, Expression, Identifier, Operator};
use crate::ast::{AstNode, ast_enum, ast_node, child_with_field_name, children_with_field_name};

ast_enum!(
	/// Domain
	Domain,
	"boolean_domain" => BooleanDomain,
	"integer_domain" => IntegerDomain,
	"any_domain" => AnyDomain,
	"matrix_domain" => MatrixDomain,
	"domain_operation" => DomainOperation,
	_ => Identifier,
);

ast_node!(
	/// Boolean domain
	BooleanDomain,
);

ast_node!(
	/// Integer domain
	IntegerDomain,
	domain,
);

impl<'tree> IntegerDomain<'tree> {
	/// Get the range expressions of domain
	pub fn domain(&self) -> Children<'tree, Expression<'tree>> {
		children_with_field_name(self, "member")
	}
}

ast_node!(
	/// Type is inferred for RHS
	AnyDomain,
);

ast_node!(
	/// Matrix domain
	MatrixDomain,
	indexes,
	base,
);

impl<'tree> MatrixDomain<'tree> {
	/// Get the indexes of this matrix domain
	pub fn indexes(&self) -> Children<'tree, Domain<'tree>> {
		children_with_field_name(self, "index")
	}

	/// Get the base domain of this matrix domain
	pub fn base(&self) -> Domain<'tree> {
		child_with_field_name(self, "base")
	}
}

ast_node!(
	/// Domain operation
	DomainOperation,
	operator,
	left,
	right,
);

impl<'tree> DomainOperation<'tree> {
	/// Get the operator of this domain operation
	pub fn operator(&self) -> Operator<'tree> {
		child_with_field_name(self, "operator")
	}

	/// Get the left operand of this domain operation
	pub fn left(&self) -> Domain<'tree> {
		child_with_field_name(self, "left")
	}

	/// Get the right operand of this domain operation
	pub fn right(&self) -> Domain<'tree> {
		child_with_field_name(self, "right")
	}
}

#[cfg(test)]
mod tests {
	use expect_test::expect;

	use crate::ast::tests::check_ast_eprime;

	#[test]
	fn test_domain_operation() {
		check_ast_eprime(
			"given a: int(1..2) union int(3..4)",
			expect![[r#"
    EPrimeModel(
        Model {
            items: [
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: DomainOperation(
                            DomainOperation {
                                cst_kind: "domain_operation",
                                operator: Operator {
                                    cst_kind: "union",
                                    name: "union",
                                },
                                left: IntegerDomain(
                                    IntegerDomain {
                                        cst_kind: "integer_domain",
                                        domain: [
                                            SetConstructor(
                                                SetConstructor {
                                                    cst_kind: "set_constructor",
                                                    operator: Operator {
                                                        cst_kind: "..",
                                                        name: "..",
                                                    },
                                                    left: IntegerLiteral(
                                                        IntegerLiteral {
                                                            cst_kind: "integer_literal",
                                                        },
                                                    ),
                                                    right: IntegerLiteral(
                                                        IntegerLiteral {
                                                            cst_kind: "integer_literal",
                                                        },
                                                    ),
                                                },
                                            ),
                                        ],
                                    },
                                ),
                                right: IntegerDomain(
                                    IntegerDomain {
                                        cst_kind: "integer_domain",
                                        domain: [
                                            SetConstructor(
                                                SetConstructor {
                                                    cst_kind: "set_constructor",
                                                    operator: Operator {
                                                        cst_kind: "..",
                                                        name: "..",
                                                    },
                                                    left: IntegerLiteral(
                                                        IntegerLiteral {
                                                            cst_kind: "integer_literal",
                                                        },
                                                    ),
                                                    right: IntegerLiteral(
                                                        IntegerLiteral {
                                                            cst_kind: "integer_literal",
                                                        },
                                                    ),
                                                },
                                            ),
                                        ],
                                    },
                                ),
                            },
                        ),
                        wheres: [],
                    },
                ),
            ],
        },
    )
"#]],
		)
	}

	#[test]
	fn test_integer_domain() {
		check_ast_eprime(
			r#"
            given a: int(1..10)
            given i: int(1,3,5..10,15..20)
            given j: int
            "#,
			expect!([r#"
    EPrimeModel(
        Model {
            items: [
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: IntegerDomain(
                            IntegerDomain {
                                cst_kind: "integer_domain",
                                domain: [
                                    SetConstructor(
                                        SetConstructor {
                                            cst_kind: "set_constructor",
                                            operator: Operator {
                                                cst_kind: "..",
                                                name: "..",
                                            },
                                            left: IntegerLiteral(
                                                IntegerLiteral {
                                                    cst_kind: "integer_literal",
                                                },
                                            ),
                                            right: IntegerLiteral(
                                                IntegerLiteral {
                                                    cst_kind: "integer_literal",
                                                },
                                            ),
                                        },
                                    ),
                                ],
                            },
                        ),
                        wheres: [],
                    },
                ),
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: IntegerDomain(
                            IntegerDomain {
                                cst_kind: "integer_domain",
                                domain: [
                                    IntegerLiteral(
                                        IntegerLiteral {
                                            cst_kind: "integer_literal",
                                        },
                                    ),
                                    IntegerLiteral(
                                        IntegerLiteral {
                                            cst_kind: "integer_literal",
                                        },
                                    ),
                                    SetConstructor(
                                        SetConstructor {
                                            cst_kind: "set_constructor",
                                            operator: Operator {
                                                cst_kind: "..",
                                                name: "..",
                                            },
                                            left: IntegerLiteral(
                                                IntegerLiteral {
                                                    cst_kind: "integer_literal",
                                                },
                                            ),
                                            right: IntegerLiteral(
                                                IntegerLiteral {
                                                    cst_kind: "integer_literal",
                                                },
                                            ),
                                        },
                                    ),
                                    SetConstructor(
                                        SetConstructor {
                                            cst_kind: "set_constructor",
                                            operator: Operator {
                                                cst_kind: "..",
                                                name: "..",
                                            },
                                            left: IntegerLiteral(
                                                IntegerLiteral {
                                                    cst_kind: "integer_literal",
                                                },
                                            ),
                                            right: IntegerLiteral(
                                                IntegerLiteral {
                                                    cst_kind: "integer_literal",
                                                },
                                            ),
                                        },
                                    ),
                                ],
                            },
                        ),
                        wheres: [],
                    },
                ),
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: IntegerDomain(
                            IntegerDomain {
                                cst_kind: "integer_domain",
                                domain: [],
                            },
                        ),
                        wheres: [],
                    },
                ),
            ],
        },
    )
"#]),
		);
	}

	#[test]
	fn test_boolean_domain() {
		check_ast_eprime(
			r#"
                given x: bool
                given x, y: bool
            "#,
			expect!([r#"
    EPrimeModel(
        Model {
            items: [
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: BooleanDomain(
                            BooleanDomain {
                                cst_kind: "boolean_domain",
                            },
                        ),
                        wheres: [],
                    },
                ),
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: BooleanDomain(
                            BooleanDomain {
                                cst_kind: "boolean_domain",
                            },
                        ),
                        wheres: [],
                    },
                ),
            ],
        },
    )
"#]),
		);
	}

	#[test]
	fn test_matrix_domain() {
		check_ast_eprime(
			"given simple: matrix indexed by [int(1..4)] of bool",
			expect![[r#"
    EPrimeModel(
        Model {
            items: [
                ParamDeclaration(
                    ParamDeclaration {
                        cst_kind: "param_decl",
                        names: [
                            Identifier {
                                cst_kind: "identifier",
                            },
                        ],
                        domain: MatrixDomain(
                            MatrixDomain {
                                cst_kind: "matrix_domain",
                                indexes: [
                                    IntegerDomain(
                                        IntegerDomain {
                                            cst_kind: "integer_domain",
                                            domain: [
                                                SetConstructor(
                                                    SetConstructor {
                                                        cst_kind: "set_constructor",
                                                        operator: Operator {
                                                            cst_kind: "..",
                                                            name: "..",
                                                        },
                                                        left: IntegerLiteral(
                                                            IntegerLiteral {
                                                                cst_kind: "integer_literal",
                                                            },
                                                        ),
                                                        right: IntegerLiteral(
                                                            IntegerLiteral {
                                                                cst_kind: "integer_literal",
                                                            },
                                                        ),
                                                    },
                                                ),
                                            ],
                                        },
                                    ),
                                ],
                                base: BooleanDomain(
                                    BooleanDomain {
                                        cst_kind: "boolean_domain",
                                    },
                                ),
                            },
                        ),
                        wheres: [],
                    },
                ),
            ],
        },
    )
"#]],
		)
	}
}
