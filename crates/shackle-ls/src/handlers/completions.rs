...
use shackle_hir::{
	CompilerDatabase, Expression, PatternTy, input::ModelFile, source::find_expression,
};
...
impl RequestHandler<Completion, (ModelFile, Position)> for CompletionsHandler {
	fn prepare(
	...
	fn execute(
		...
		let Some(expression) = find_expression(db, model_ref, byte_offset) else {
			return Ok(None);
		};
