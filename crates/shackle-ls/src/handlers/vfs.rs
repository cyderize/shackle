...
pub(crate) fn on_document_changed(db: &mut LanguageServerDatabase, params: DidChangeTextDocumentParams) {
	let file = uri_to_path(&params.text_document.uri);
	db.manage_file(
		&file,
		&params
...
pub(crate) fn on_document_closed(db: &mut LanguageServerDatabase, params: DidCloseTextDocumentParams) {
	let file = uri_to_path(&params.text_document.uri);
