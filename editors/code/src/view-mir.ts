...
const viewMirRequest = new RequestType<
	TextDocumentPositionParams,
	string,
	void
>("shackle-ls/viewMir")

class MirProvider implements TextDocumentContentProvider {
	readonly eventEmitter = new EventEmitter<Uri>()
	readonly uri: Uri = Uri.parse("shackle-ls://viewMir/mir.mzn")
	...
	provideTextDocumentContent(
		...
		return this.client.sendRequest(viewMirRequest, params, ct)
	}
...
let provider: MirProvider | null = null
export async function handleMirViewCommand(client: LanguageClient) {
	if (!provider) {
		provider = new MirProvider(client, () => {
			provider = null
		})
	}
